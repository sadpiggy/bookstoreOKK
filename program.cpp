...
void program::run() {//这个最后改
    ...
    while(getline(cin,a))
    {
        ...
        if(strcmp(token.c_str(),"select")==0){
            if(get_token_num(a)!=2){cout<<"Invalid"<<endl;return;}
            string token_2=get_token(a,2);
            select(token_2,*user);
            write_out();continue;}       //已调鲁邦
        if(strcmp(token.c_str(),"modify")==0){           //已调
            modify(a,*user);
            write_out();continue;}
        if(strcmp(token.c_str(),"import")==0){import(a,*user);write_out();continue;}       //已调
        if(strcmp(token.c_str(),"show")==0){show(a,*user);write_out();continue;}           //已调
        if(strcmp(token.c_str(),"buy")==0){
            if(get_token_num(a)!=3){cout<<"Invalid"<<endl;return;}
            string ISBN_=get_token(a,2);
            string token_3=get_token(a,3);
            if(token_3[0]-'0'<0||token_3[0]-'0'>9){cout<<"Invalid"<<endl;return;}
...
void program::users(const string& aa,people& user) {//哲理绝对会内存泄漏，之后记得处理//改为string 之后就不会了  //所以就不用且不能delete了
   string token_1=get_token(aa,1);
    ...
    {
        if(get_token_num(aa)!=1){cout<<"Invalid"<<endl;return;}
    ...
    {
        if(get_token_num(aa)!=4){cout<<"Invalid"<<endl;/*delete token_1*/;return;}//可能出问题//还是得严格鲁棒
        user.user_register(get_token(aa,2),get_token(aa,3),get_token(aa,4),people_system);
        return;
    }
    if(strcmp(get_token(aa,1).c_str(),"delete")==0)
    {
        if(get_token_num(aa)!=2){cout<<"Invalid"<<endl;return;}
...
void program::show(const string &aa, people &user) {//我有有个大大的问题，show keyword=...的时候，万一有好几本书，怎么办，用map维护
   ...
   // if(token[0]=='f')
    if(strcmp(token.c_str(),"finance")==0)
    {
        if(get_token_num(aa)==2)
